...
| v1.4.0 | 2026-08-27 | Claude | `integration/`(외부 시스템 연동 계약) 신설 — §2 구조도·구분표, §3.5 문서 목록에 등록. `계측 데이터 API 명세(ThingsBoard)`가 `datasets/`에 있었는데 **그 폴더는 실측 xlsx를 담는 곳**이라 근거 태그 `[데이터셋 …]`과 성격이 어긋났다. 파일명도 `API명세.pdf`에서 시스템·날짜를 담은 이름으로 바꿨다 — 계약이 바뀌는 문서이고, 곧 다른 API 명세가 따라온다 |
...
├── integration/       외부 시스템 연동 계약 (그쪽이 우리에게 무엇을 주는가)
└── datasets/          데이터셋
```
...
| 연동 계약 | [integration/](integration/) | 우리가 **붙어야 하는 외부 시스템**의 계약. 원문도 우리 설계도 아니다 |
...
### 3.4 `integration/` — 외부 시스템 연동 계약

| 문서 | 담는 내용 | 주로 쓰는 상황 |
|------|-----------|----------------|
| [README.md](integration/README.md) | **우리 쪽 구조.** 왜 붙였는지 · 계층 4개의 위치 · 자격증명을 어디 두는지 · 폴백 규약(`pending`/`live`/`fallback`) · 미수신 채널 · 아직 하지 않은 것 | 연동 코드를 만지거나, 화면이 왜 `시연 데이터`라 적는지 볼 때 |
| [ThingsBoard_Telemetry_API_2026-08-24.pdf](integration/ThingsBoard_Telemetry_API_2026-08-24.pdf) | **계측 데이터 API**(ThingsBoard CE 4.3.1.3). 엔드포인트 8종 · 채널 사전 14종 · 결측 규약 · 오류 처리 · 매핑 지침. 14쪽 | 계측을 fixture에서 실 API로 옮길 때 |

**PDF가 계약이고 `README.md`가 우리 구조다.** 저쪽이 약속한 것과 우리가 그것을 어떻게 받았는지는 다른 문서다.

**원문도 우리 설계도 아니다.** `applications/`는 사업 신청 문서이고 `specs/`는 우리가 결정한 것인데, 이 문서는 **저쪽 시스템이 우리에게 약속한 것**이다. `datasets/`에 있던 것을 옮겼다 — 그 폴더는 실측 xlsx를 담는 곳이라 근거 태그 `[데이터셋 …]`의 뜻과 어긋났다.

> **이 API는 계측만 준다.** 이상 점수·예측·알람·설비 상태·공정 구성·사업장 메타는 주지 않는다(문서 §1.1). 그 항목들은 여전히 fixture다.
>
> **파일명에 날짜가 붙는 이유** — 접속 주소·ThingsBoard 버전·채널 구성이 바뀔 수 있는 문서다(§2). `applications/`의 원문 셋은 확정된 문서라 날짜가 없다.
