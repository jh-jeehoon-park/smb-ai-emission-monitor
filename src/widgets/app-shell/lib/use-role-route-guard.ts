 ...
 * (이 저장소에서 두 번 터졌다).
 *
 * `own-municipality`(기초지자체)는 **아직 필터가 없다** `[사용자 지시 2026-08-20]` — 구현할 때
 * `scope=municipality&municipality=…`를 같은 방식으로 얹는다. 지금은 전 사업장과 같이 흐른다.
 *
 * **예전에 `scope=province&province=…`로 적어 두었던 것은 낡았다.** 개명으로 범위가 시·도에서
 * 시·군·구로 한 단 좁아졌다 `[사용자 요청 2026-08-24]` — 그대로 따르면 개명이 없앤 광역 범위를
 * 되살린다. 설계는 `docs/specs/screens/SCR-GU-001-관내감독현황.md` §8이 갖고 있다.
 *
