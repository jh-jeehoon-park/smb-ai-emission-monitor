 ...
 * 시연에서 **전환할 수 있는** 역할.
 *
 * 기초지자체가 빠진다 `[사용자 지시 2026-08-20]`. 관할 범위 필터를 아직 만들지 않았으므로
 * 전환해도 시스템 관리자와 화면이 같다 — **전환해 봐야 구분되지 않는 것을 고를 수 있게
 * 두면 없는 기능이 있는 것처럼 읽힌다.** 조회 전용 역할이 같은 이유로 막혀 있던 자리를 잇는다.
 *
 * **개명으로 이 제약이 풀릴 여지가 생겼다** `[사용자 요청 2026-08-24]`. 시·군·구로 좁히면
 * 실제로 갈린다 — 경북 안동 2 · 구미 1 · 칠곡 1 · 포항 1 · 경산 1 / 경기 평택·시흥·수원·광주
 * 각 1이다. 필터를 만들면 전환을 열 수 있다.
 *
 * **접근 권한(`SCREEN_ROLES`)은 건드리지 않는다.** 기초지자체가 볼 수 있는 화면의 정의는
...
export const ROLE_SWITCH_BLOCKED_REASON =
  '기초지자체는 관할 시·군·구 범위를 아직 구현하지 않아 시연에서 전환하지 않습니다';
