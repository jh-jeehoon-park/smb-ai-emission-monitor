...
export const SCREEN_ROLES: Record<string, readonly Role[]> = {
  ...
  'SCR-CO-001': ROLES,
  /*
   * **안내·오류 화면도 역할로 가리지 않는다** `[사용자 요청 2026-09-15]`.
   *
   * 로그인과 같은 계열이되 이유가 다르다 — 그쪽은 역할이 «아직 없는» 화면이고 이쪽은 역할이
   * **무엇이든 만날 수 있는** 화면이다. 없는 주소(404)와 무너진 렌더(500)는 역할을 가리지
   * 않고, 403은 **막힌 역할에게 보여야 하므로** 그 역할에 닫아 두면 자기가 왜 못 보는지를
   * 볼 수 없게 된다.
   *
   * **라우트가 없다.** Next 특수 파일(`not-found.tsx`·`error.tsx`·`global-error.tsx`)과 셸
   * 안의 `RoleGate`가 연다. 여기 등재하는 이유는 `user.test.ts`가 이 표와 `screens.md` §5
   * 매트릭스를 칸 단위로 대조하기 때문이다 — 표에만 넣으면 그 테스트가 깨진다(`SCR-GU-001`이
   * 미구현이던 시절에 세운 선례다).
   */
  'SCR-CO-002': ROLES,
  /*
