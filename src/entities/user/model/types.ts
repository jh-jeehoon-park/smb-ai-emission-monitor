/**
 * 사용자 3종. **2026-08-20 회의가 정했다** — 사업장 / 기초지자체 공무원 / 시스템 관리자.
 *
 ...
 *
 * `own-municipality`는 **아직 동작하지 않는다.** 기초지자체 구현을 뒤로 미뤘으므로
 * (`[사용자 지시 2026-08-20]`) 타입에만 있고 필터가 없다 — 최종 형태를 미리 두어
 * 권한 매트릭스·문서를 두 번 고치지 않으려는 것이다.
 *
 * **시·도가 아니라 시·군·구다** `[사용자 요청 2026-08-24: 지자체 → 기초지자체]`. 이름이
 * `own-province`였을 때는 광역을 뜻했는데, 기초지자체는 기초자치단체(시·군·구)라 범위가
 * 한 단 좁아진다. 지도 경계(`korea-provinces.ts`)는 시·도 17개뿐이라 지도 표현은 별도 작업이다.
 */
export type RoleScope = 'all-sites' | 'own-municipality' | 'own-site';
