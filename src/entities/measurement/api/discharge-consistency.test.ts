 ...
 * `방류 중단`이라 적는 구간에도 유량이 412 언저리로 흘렀다 `[사용자 결정 2026-08-28]`.
 *
 * **누적 배출량이 그때의 근거였고 지금은 없다** `[회의 2026-09-08]`. 그래도 이 불변식은
 * 남는다 — `금일 배출 현황`과 `현황판`의 유량 차트가 **바닥에 닿은 선으로 «멈췄다»를
 * 말하기 때문**이다. 유량이 흐르는데 리본이 «중단»이라 적으면 그 그림이 거짓이 된다.
 *
