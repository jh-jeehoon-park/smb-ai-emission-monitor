 ...
 * 회색 강조(`bg-surface-3`)는 본문 배경(#f3f4f5)과 붙어 어느 탭이 선택됐는지 보이지 않는다.
 *
      ...
      {sites.map((site) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex shrink-0 cursor-pointer items-center gap-1.5 rounded-nested border px-2.5 py-1.5 text-[12px]',
              'transition-colors duration-200',
              selected
                ? 'border-card-border bg-surface font-semibold text-fg shadow-panel'
                : 'border-transparent text-fg-subtle hover:bg-surface/70 hover:text-fg-muted',
            )}
          >
            {/*
