 ...
 * 뷰박스 단위로 적은 것이 **같은 비율로 함께** 커진다(경북에서 1.27배).
 *
 * 도형·핀·라벨은 그래야 맞다 — 확대해서 보는 중이다. **툴팁은 아니다**: 지도 위에 얹힌 판이라
 * 축척과 무관하게 같은 크기여야 읽힌다. 여기서 되돌리지 않고 툴팁 쪽에서 화면 배율을 재
 * 되돌린다(`site-map/lib/use-screen-unit.ts`) — 이 함수가 손대면 아래 경고에 걸린다.
 *
