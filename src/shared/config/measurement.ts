 ...
 * 화면이 표본을 얹는 격자 주기.
 *
 * **장비의 실제 수집 주기는 1분이다** `[사용자 확인 2026-08-27]` `[INC-111]`. 그런데 장비가
 * **2026-08-27 11:14 KST부터** 1분으로 바뀌어 그 이전 22시간은 아직 5분 간격이다. 1분 격자에
 * 얹으면 24시간 창의 1,440칸 중 **25.6%만 차고 미니차트 선이 267조각으로 끊긴다**(실측).
 * 결측을 잇지 않는 것이 규칙이므로(E4) 그림이 틀린 것이 아니라 **데이터가 아직 성기다.**
 *
 * 한때 임시로 5분을 썼다 `[사용자 요청 2026-08-27]`. **하루 만에 되돌렸다** — 백엔드가 같은 날
 * 24시간 전 구간을 1분으로 소급해 채워(1,429점) 성긴 문제가 사라졌고, 더 나아가 **5분 격자가
 * 해로워졌다**: 옛 5분 데이터가 지워지지 않고 1분 백필과 한 시계열에 섞여 있어 **5분 배수
 * 시각(`:00`·`:05`…)에만 옛 값이 남아 있다.** 5분 격자는 정확히 그 표본만 골라 얹으므로
 * S-08이 24시간 내내 `방류 중단`으로 보였다(그 시각 288개 중 100%가 방류 0, 그 밖은 1%).
 *
