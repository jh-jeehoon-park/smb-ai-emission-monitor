 ...
 *
 * ## **화면은 더 이상 이 만수위를 말하지 않는다** `[사용자 결정 2026-09-15]`
 *
 * 범위는 fixture가 수위 거동을 만드는 데 쓰는 값으로 남고(`fixtures.ts`가 목표 수위를
 * 만수위의 비율로 잡는다), **화면이 그것을 «만수위 3m»이라 인쇄하고 축으로 고정하던 것만
 * 걷었다.** 두 가지가 겹쳐 그 표기가 거짓이 됐기 때문이다.
 *
 * | 원천 | 수위 범위 | 만수위 3m 초과 |
 * |---|---|---|
 * | fixture | 1.12 ~ 2.60 m | 0건 / 12,850 |
 * | 계측 서버 | 3.41 ~ 3.89 m | 사실상 전부 (2026-09-11 실측) |
 *
 * 즉 `0~3`은 **fixture에 맞춰 잡힌 값**이고 실측이 그 규약을 지키지 않는다 — 우리가 정한
 * 임시값이 판정 기준처럼 화면에 찍혀 있었고, `SCR-OP-011` §7.4가 이미 *"만수위를 알아야
 * 성립하는데 그 값도 우리가 정한 것이라 임시값 위에 임시값을 쌓는다"* 로 게이지 표기를
 * 기각해 둔 것과 같은 이유다. 자리를 대신하는 것은 **오늘 관측 범위**다.
 *
 * `[TBD-57]`이 확정되면 만수위를 화면에 되살린다.
 */
