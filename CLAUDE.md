...
| 화면이 읽는 곳 | `useSiteSeries(siteId)` **하나뿐이다.** `getMeasurementSeries`를 화면에서 직접 부르지 않는다 |
...
> **`A1`(실 API 연동 시 Mock 전량 제거)이 여기서 예외가 된다.** 폴백이 fixture를 요구하므로 남긴다 `[사용자 결정 2026-08-27]` — fixture는 *임시물*이 아니라 **서버 미도달 시의 정식 대체 원천**이다. **배포본은 다시 폴백으로 돈다** `[사용자 확인 2026-09-15]` — 주소가 사내망으로 돌아갔다(`192.168.50.25:28080`). 한때 공인 IP로 포트포워딩되어 *"사설망 http라 배포본에서는 애초에 닿지 않는다"* 가 깨졌었는데(2026-09-01), 그 포트포워딩이 죽었다. **폴백을 남긴 판단은 두 번 다 맞았다** — 근거가 «구조적으로 못 닿는다» ↔ «재기동·토큰 만료·순단으로 실패가 일상이다» 사이를 오갔을 뿐이다(`docs/integration/README.md` §4.2·§5).
