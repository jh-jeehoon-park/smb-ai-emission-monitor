...
| 범위 | **프론트엔드 화면 + 계측 조회.** 자체 백엔드는 없고, **계측 서버(ThingsBoard)에만 붙는다**(2026-08-27) |
| 데이터 | **계측은 실측**(닿을 때) · 나머지 전부 fixture. 계측 서버에 닿지 않으면 **fixture로 자동 폴백**한다 `[사용자 결정 2026-08-27]` |

### 계측만 실 API다 (2026-08-27)

계측 서버 하나에 붙었고 **나머지 도메인은 그대로 fixture다.** 그쪽이 주지 않기 때문이다 — 이상 점수·예측·알람·설비 상태·공정 구성·사업장 메타는 이 API의 범위 밖이다.

| | |
|---|---|
| 계약 | `docs/integration/ThingsBoard_Telemetry_API_2026-08-24.pdf` (14쪽) |
| **우리 쪽 구조** | [`docs/integration/README.md`](docs/integration/README.md) — 계층·자격증명 위치·폴백 규약. **연동 코드를 만지기 전에 읽는다** |
| 화면이 읽는 곳 | **둘뿐이다** — `useSiteSeries(siteId)`(24시간 격자, 역사)와 `useSiteLatest(siteId)`(그 사업장 수집 주기로 오는 **지금 값** 한 점) `[사용자 요청 2026-09-16]`. 둘을 가른 이유는 갱신 주기가 달라서다: 한 객체에 담았더니 5초마다 화면 전체가 다시 계산돼 갱신마다 **최대 4초 멈췄다**(`docs/integration/README.md` §4.4). `getMeasurementSeries`를 화면에서 직접 부르지 않는다 |
| 자격증명 | `.env.local` (서버 전용). **`NEXT_PUBLIC_`을 붙이지 않는다** — 붙이면 번들에 실린다 |

**반드시 지키는 셋** — 어기면 화면이 조용히 거짓말한다.

1. **`?? 0`을 쓰지 않는다.** 미가동 구간의 전류 0은 사실이고 두절 구간의 빈칸은 모름이다(**E4**)
2. **서버에서 온 시각은 `kstIsoFromEpoch()`를 지난다.** 이 저장소의 시각 문자열은 `Z`가 붙지만 **UTC가 아니라 KST 벽시계**다 — 실 epoch을 그대로 넣으면 9시간 어긋난 값에 KST 라벨이 붙는다
3. **구간 길이는 표본 수가 아니라 분으로 적는다**(`minutesToSamples`). 수집 주기가 바뀌면 박아 둔 표본 수가 조용히 다른 시간을 뜻하게 된다 — 5분→1분에서 실제로 사건 3시간이 36분으로, 두절 55분이 11분으로 줄었다 `[INC-111]`

> **`A1`(실 API 연동 시 Mock 전량 제거)이 여기서 예외가 된다.** 폴백이 fixture를 요구하므로 남긴다 `[사용자 결정 2026-08-27]` — fixture는 *임시물*이 아니라 **서버 미도달 시의 정식 대체 원천**이다. **주소는 `.env.local`이 정본이고 여기 적지 않는다** `[사용자 확인 2026-09-16]` — 사내망 ↔ 공인 IP 사이를 **네 번** 오갔고(마지막은 `211.44.234.147:28080`, 공인) 그때마다 번호도 바뀌어, 문서에 박아 두면 다음 사람이 죽은 주소로 되돌린다. **폴백을 남긴 판단은 세 번 다 맞았다** — 근거가 «구조적으로 못 닿는다» ↔ «재기동·토큰 만료·순단으로 실패가 일상이다» 사이를 오갔을 뿐이고 어느 쪽이든 폴백이 필요하다(`docs/integration/README.md` §4.2·§5).
...
| **이상 구간 최소 지속 시간** | `20분` | TBD-02 관련 — **구간이라는 개념 자체가 우리 것이다.** 이상 점수가 주의 경계 위로 이어진 구간을 세는 화면(`SCR-OP-002 이상 탐지`)이 쓴다. 짧게 잡으면 잡음이 경계를 스칠 때마다 구간이 늘어 목록이 읽히지 않고, 길게 잡으면 짧고 날카로운 이상이 사라진다. `PROVISIONAL_ANOMALY_RUN_MIN_MINUTES` 하나만 바꾸면 전 화면이 따라온다 `[사용자 요청 2026-09-08]` |
| **방류 의심 최소 지속 시간** | `60분` | TBD-46 — 원문은 방법만 주고(*"전류 발생 시기와 유량 발생 시기를 비교"* `[원문 발표 p.13]`) 몇 분부터 의심인지 정하지 않았다. **한때 `= 12`(표본)였고 주석은 «1시간»이라 적었다** — 5분 주기 전제였는데 수집 주기가 1분으로 확정되며(`INC-111`) 실제 임계가 조용히 12분으로 줄었다. 지금은 `PROVISIONAL_IDLE_DISCHARGE_MIN_MINUTES`로 **분**이다 `[사용자 지적 2026-09-08]` |
| 방류 수조 수위 단위·만수위 | `m` · `0~3m` (**화면은 만수위를 말하지 않는다**) | TBD-57 — **항목은 원문에 있고 사양이 없다.** `[원문 p.1]`이 `배출 데이터`를 `유량 · 수위 · 방류 여부` 3종으로 규정하고 활용 목적을 *"배출량 및 부하량 산정 기반 데이터"* 라 적는데, 단위·범위·측정 방식이 어디에도 없다(`data-dictionary.md` §4.2·§10 #12). `[원문 p.53]`의 수위스위치 7대는 ON/OFF라 이 수치가 아니다. `SCR-OP-011 금일 배출 현황`이 쓰며 `PROVISIONAL_LEVEL_UNIT`·`PROVISIONAL_LEVEL_RANGE` 둘만 바꾸면 전 화면이 따라온다 `[사용자 결정 2026-08-28]`. **범위는 이제 fixture의 거동을 만드는 값으로만 남는다** `[사용자 결정 2026-09-15]` — 계측 서버 실측(3.41~3.89m)이 `3m`를 넘어 **화면이 «만수위»를 인쇄하고 축으로 고정하던 것을 걷었다**(관측 범위로 대체). `SCR-OP-011` §7.6 |
