...
| 방류 수조 수위 단위·만수위 | `m` · `0~3m` | TBD-57 — **항목은 원문에 있고 사양이 없다.** `[원문 p.1]`이 `배출 데이터`를 `유량 · 수위 · 방류 여부` 3종으로 규정하고 활용 목적을 *"배출량 및 부하량 산정 기반 데이터"* 라 적는데, 단위·범위·측정 방식이 어디에도 없다(`data-dictionary.md` §4.2·§10 #12). `[원문 p.53]`의 수위스위치 7대는 ON/OFF라 이 수치가 아니다. `SCR-OP-011 금일 배출 현황`이 쓰며 `PROVISIONAL_LEVEL_UNIT`·`PROVISIONAL_LEVEL_RANGE` 둘만 바꾸면 전 화면이 따라온다 `[사용자 결정 2026-08-28]` |
