...
> **`A1`(실 API 연동 시 Mock 전량 제거)이 여기서 예외가 된다.** 폴백이 fixture를 요구하므로 남긴다 `[사용자 결정 2026-08-27]` — fixture는 *임시물*이 아니라 **서버 미도달 시의 정식 대체 원천**이다. **배포본도 실측을 본다** `[사용자 확인 2026-09-01]` — 공인 IP로 포트포워딩되어 한때의 근거(*"사설망 http라 배포본에서는 애초에 닿지 않는다"*)가 깨졌다. 그래도 폴백은 남는다: 재기동·토큰 만료·순단으로 실패가 일상이기 때문이다(`docs/integration/README.md` §5).
